...
from ...utils.tools import (
    EnvironmentUtils, TimeUtils, FileUtils, ProcessUtils,
    run_async_safely, validate_required, ensure_dir, get_current_pid
)
...
class ServiceRegistry:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._lock_fd = None
        self._enable_process_lock = self.nacos_config.get('registration', {}).get(
    ...
    async def initialize_async(self):
        ...
        try:
            ...
            logger.info(
                f"Nacos client initialized: {self.nacos_config.get('server', 'localhost:8848')}")
    ...
    async def register_service_async(self) -> bool:
        ...
        try:
            if self._mock_mode:
                ...
                logger.info(
                    f"Service registered successfully (MOCK MODE): "
                    f"{service_info['service_name']}:{service_info['port']}")
            ...
            logger.info(
                f"Service registered successfully: {service_info['service_name']}:"
                f"{service_info['port']}")
            ...
            if self._retry_count < self._max_retries:
                self._retry_count += 1
                logger.info(
                    f"Retrying registration ({self._retry_count}/{self._max_retries}) in "
                    f"{self._retry_interval}s...")
                import asyncio
    ...
    async def deregister_service_async(self) -> bool:
        ...
        try:
            if self._mock_mode:
                ...
                logger.info(
                    f"Service deregistered successfully (MOCK MODE): "
                    f"{service_info['service_name']}:{service_info['port']}")
                return True
            ...
            if service_info:
                ...
                logger.info(
                    f"Service deregistered successfully: {service_info['service_name']}:"
                    f"{service_info['port']}")
    ...
    async def update_instance_async(self, **kwargs) -> bool:
        ...
        try:
            ...
            if service_info:
                ...
                logger.info(
                    f"Instance updated successfully: {service_info['service_name']}:"
                    f"{service_info['port']}")
                return True
    ...
    def _acquire_file_lock(self, file_handle) -> bool:
        ...
        try:
            if HAS_FCNTL:
                ...
                # Fallback: no proper locking available
                logger.warning("No file locking mechanism available on this platform")
                return True  # Allow to proceed without locking
    ...
    def _acquire_process_lock(self) -> bool:
        ...
        if not HAS_FCNTL and not HAS_MSVCRT:
            logger.warning(
                "Process locking is enabled but no file locking mechanism is available. "
        ...
        try:
            ...
            # Use cross-platform file lock
            if not self._acquire_file_lock(self._lock_fd):
                if self._lock_fd:
                    self._lock_fd.close()
                    self._lock_fd = None
                ...
                logger.warning("Another process is already managing this service instance")
                return False
            ...
            # Check existing lock file information
            self._check_existing_lock()
            logger.warning(f"Failed to acquire process lock: {e}")
            return False
                    ...
                    if line.startswith('pid='):
                        try:
                            ...
                            if not ProcessUtils.is_process_running(existing_pid):
                                logger.warning(
                                    f"Process {existing_pid} is no longer running, lock may be "
                                    f"stale")
    ...
    def _release_process_lock(self):
        ...
        if self._lock_fd:
            try:
                # Use cross-platform file lock release
